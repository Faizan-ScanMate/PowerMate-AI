...
@Composable
fun MetricCard(title: String, value: String, caption: String, modifier: Modifier = Modifier, accent: Color = Cyan) {
    SectionCard(modifier = modifier) {
        ...
        Text(caption, color = accent, fontSize = 12.sp, fontWeight = FontWeight.Medium)
    }
}

@Composable
fun StatusChip(text: String, color: Color = SuccessGreen) {
    Box(
        modifier = Modifier
...
fun MiniGraph(values: List<Float>, modifier: Modifier = Modifier) {
    ...
    Canvas(modifier = modifier.fillMaxWidth().height(80.dp)) {
        ...
        for (i in 0 until safeValues.lastIndex) {
            val x1 = i * step
            val y1 = size.height - (safeValues[i] / max) * size.height
            val x2 = (i + 1) * step
            val y2 = size.height - (safeValues[i + 1] / max) * size.height
            drawLine(
