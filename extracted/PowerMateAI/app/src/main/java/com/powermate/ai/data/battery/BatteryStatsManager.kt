...
class BatteryStatsManager(private val context: Context) {
    ...
    private fun safeIntBatteryProperty(property: Int): Int? {
        return try {
            ...
            // Per the BatteryManager API contract, Int.MIN_VALUE means "not supported on this
            // device". 0 is a perfectly valid reading (e.g. momentary zero current) and must
            // not be treated as missing data.
            value.takeIf { it != Int.MIN_VALUE }
        } catch (_: Exception) {
            null
        }
    }
    ...
    private fun safeLongBatteryProperty(property: Int): Long? {
        return try {
            ...
            value.takeIf { it != Long.MIN_VALUE }
        } catch (_: Exception) {
            null
        }
    }
